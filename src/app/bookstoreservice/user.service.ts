...
export class UserService {
 ...
 getloginStatus(email:string, password:string){
    return this.http.get("http://localhost:8082/user/logintest?email="+email+"&password="+password);
  }
  ...
  forgotPasswordUser(email:string, password:string): Observable<any>{
    const params = new HttpParams()
                .set('email', email)
                .set('password', password);
    return this.http.post("http://localhost:8082/user/forgotpassword", params);
  }
